...
public class UserRoleStore : UserStore, IUserRoleStore<User>
{
    ...
    public Task AddToRoleAsync(User user, string roleName, CancellationToken cancellationToken)
    {
        try
        {
            Role role = _context.Roles.FirstOrDefault(x => x.Name == roleName)
    ...
    {
        List<string?> roles = user.Roles.Select(x => x.Name)
            .ToList();
    ...
    public async Task<IList<User>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken)
    {
        try
        {
            List<User> users = await _context.Roles.Where(x => x.Name == roleName)
